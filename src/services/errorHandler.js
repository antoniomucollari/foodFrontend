  ...
  if (error.response) {
    ...
    if (statusCode >= 400 && statusCode < 500) {
      ...
      if (statusCode === 400) {
        ...
        showError('Unauthorized. Please log in again.', { title: 'Authentication Required' });
      } else if (statusCode === 403) {
        showError('You do not have permission to perform this action.', { title: 'Access Denied' });
      } else if (statusCode === 404) {
        showError('The requested resource was not found.', { title: 'Not Found' });
      ...
      // Server errors (500+)
      showError('Server error. Please try again later.', { title: 'Server Error' });
    } else {
      // Other errors
    ...
    // Network error
    showError('Network error. Please check your connection.', { title: 'Connection Error' });
  } else {
    // Other errors

...
const DeliveryLiveOrders = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      // This endpoint will need to be updated when the backend is ready
      // For now, we'll use a placeholder that filters by delivery person
      const response = await fetch(
        `http://localhost:8080/api/orders/delivery/incomplete-orders?deliveryId=${user?.id}&page=0&size=50`,
        {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    if (isConnected && user?.id) {
      ...
      // Handle new incomplete orders assigned to this delivery person
      const handleNewOrder = (order) => {
        console.log("New incomplete order received via WebSocket:", order);
        // Only process orders assigned to this delivery person
        if (
      ...
      const handleOrderUpdate = (order) => {
        ...
        setOrders((prev) => {
          // Check if order is still incomplete and assigned to this delivery person
          const isIncomplete = isOrderIncomplete(order);
          const isAssignedToMe =
            order.deliveryPerson?.id === user.id ||
            order.deliveryId === user.id;

          if (isIncomplete && isAssignedToMe) {
            ...
            // Remove order if it's no longer incomplete or not assigned to this delivery person
            console.log("Removing completed/unassigned order:", order.id);
